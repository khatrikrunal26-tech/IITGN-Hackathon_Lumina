...
if mode == "📚 Learning Path":
    ...
    with st.container(border=True):
        ...
        # Make description URL-safe and remove problematic characters
        img_desc = visual_desc.replace(" ", "%20").replace("\n", "%20").replace(".", "%20").replace(",", "%20").replace(":", "%20").replace("(", "").replace(")", "").replace("[", "").replace("]", "")
        
        # Better topic encoding
        img_topic = topic.replace(" ", "%20")
        
        # Improved prompt — much more likely to get actual diagrams
        diagram_url = (
            f"https://pollinations.ai/p/"
            f"clear_textbook_style_labeled_scientific_diagram_{img_topic}_"
            f"in_{subject.replace(' ', '%20')}_"
            f"{img_desc}_"
            f"simple_educational_white_background_no_extra_objects_no_people"
            f"?width=900&height=700&seed=44&nologo=true"
        )
        
        # Modern Streamlit way — should actually display the image
        st.image(diagram_url, caption=f"Visualization: {topic}", use_column_width=True)
